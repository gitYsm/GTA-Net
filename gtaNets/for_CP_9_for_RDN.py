...
if parsed.gt:
    ...
    for GTA_e in range(10001):
        ...
        if (GTA_e%2==0 and GTA_e<20) or d_out>0.6:
        ...
        if (GTA_e)%10==0:
            for number in range(5):
                a_image = rdn.predict_generator(a_batchg_lr_ex[number],steps=1)[0]
            ...
            cv2.imwrite('G_SRGAN'+'{:04}'.format(GTA_e)+'.png', cv2.vconcat([h_img1,h_img2]))
...
if parsed.pt:
    ...
    for pt_e in range(5001):
        ...
        if pt_e%20==0:
            for number in range(5):
                a_image1 = rdn.predict_generator(a_batchg_lr_ex[number],steps=1)
                a_image11 = a_image1[0]
                a_image12 = post_gen.model.predict(a_image1)[0]
            ...
            for number in range(5,10):
                a_image2 = rdn.predict_generator(a_batchg_lr_ex[number],steps=1)
                a_image21 = a_image2[0]
                a_image22 = post_gen.model.predict(a_image2)[0]
...
if parsed.test:
    ...
    rdn.load_weights('./outputs/log01/generator_weights1500.h5')
    ...
    for number in range(5,10):
        ...
        if number==5:
            ...
            h_img2 = cv2.hconcat([h_img2, a_image])
    cv2.imwrite('G_SRGAN.png', cv2.vconcat([h_img1,h_img2]))
